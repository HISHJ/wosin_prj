...
//1.총 페이지수 알기
//한 화면에 출력될 페이지수가 10개라고 해도 3페이지까지만 존재할 경우
// 1~3페이지까지만 하단에 출력해줄 수 있음
int page_ = 1;// 현재 페이지 번호

int totalCount = rDAO.selectRsrvtPagenation(rVO); //3개
System.out.println(totalCount);
int countList = totalCount-1; // 한 화면에 출력될 리스트 수
int totalPage = totalCount / countList; // 총 페이지수
int countPage = 3; //한 화면에 출력될 페이지 개수

if(totalCount > countList * totalPage){
	totalPage ++;
}	
//2. 현재 페이지 번호가 총 페이지 번호보다 크다면 => 현재 페이지를 강제로 총 페이지 번호로 치환
if(totalPage < page_){
	page_ = totalPage;
			
}

//3. 하단에 표시될 페이지 번호들 알아내는 방법
//페이지 표시방식 : 10갸의 페이지를 보이는 방식
int startPage = ((page_ - 1)/ 10)*10 +1; // 왜 1을 더하는지 생각해보자 =>1
int endPage = startPage + countPage -1; // 왜 1을 빼는지 생각해보자 => 10

//3-1. 마지막 페이지는 총 페이지수로 대체
if(endPage > totalPage){
	endPage = totalPage;			
}

//[paging]
//4. 출력 (페이지 번호가 연달아서 출력됨)
%>
<ul class="paging" style="display:flex;align-items:center;justify-content: center; ">
<%for(int iCnt = startPage; iCnt <= endPage; iCnt ++){
	if (iCnt == page_) {%>
	<li class="active" style="background-color:#fff;border:1px solid #000;color:#000;"><a href="http://localhost/group2_prj/reservation/ticket_page_tryPagenation.jsp?<%=countList %>" onClick="go_page(<%=iCnt%>)"><%=iCnt%></a></li>
<%}else{%>
	<li class="active" ><a href="#" onClick="go_page(<%=iCnt%>)"><%=iCnt%></a></li>
